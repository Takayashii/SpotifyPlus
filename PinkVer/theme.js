// SpotifyPlus - theme.js (final release)
// Branding: SpotifyPlus startup screen.
...
(function AmbientGlass() {
  ...
  const HITBOX_STYLE_TEXT = `
      ...
      .ag-dock-hitbox:hover {
        ...
        box-shadow: 0 0 20px rgba(219, 95, 213, 0.3);
      }
  ...
  function renderLibraryGrid() {
    ...
    if (_currentType === 'playlists') {
      const createCard = document.createElement('button');
      createCard.type = 'button';
      createCard.className = 'ag-lib-item ag-create-playlist-card';
      
      const createCover = document.createElement('div');
      createCover.className = 'ag-lib-item-cover ag-library-art-placeholder';
      createCover.innerHTML = `
        <svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: #d402db;">
      ...
      createName.className = 'ag-lib-item-name';
      createName.textContent = 'Create playlist';

      createCard.append(createCover, createName);
      ...
      createCard.addEventListener('click', async (e) => {
        ...
        try {
          const rootlist = window.Spicetify?.Platform?.RootlistAPI || window.Spicetify?.Platform?.RootPlaylistAPI;
          if (rootlist && typeof rootlist.createPlaylist === 'function') {
            
            const response = await rootlist.createPlaylist("Your New Playlist", {});
            
            if (response && response.uri) {
              safeNavigate(response.uri);
            } else if (response && typeof response === 'string' && response.startsWith('spotify:')) {
              safeNavigate(response);
            }
            
            if (window.Spicetify?.showNotification) {
              window.Spicetify.showNotification("Playlist created successfully!");
            }
          } else {
            console.error("API z Testu 1 nie jest dostępne w tym kontekście.");
          }
        } catch (err) {
          console.error("Błąd podczas tworzenia i przechodzenia do playlisty:", err);
        }
      });
    ...
    if (!items.length && _currentType !== 'playlists') {
  ...
  function triggerStartupAnimation() {
    ...
    s.innerHTML = `
      <style>
        ...
        .ag-splash-glow { position: absolute; width: 600px; height: 600px; background: radial-gradient(circle, rgba(243, 30, 232, 0.4) 0%, rgba(235, 79, 235, 0.1) 40%, transparent 70%); filter: blur(60px); animation: ag-glow-cinematic 8s infinite alternate ease-in-out; pointer-events: none; }
        .ag-splash-content { position: relative; z-index: 2; display: flex; flex-direction: column; align-items: center; gap: 20px; }
        .ag-splash-title { color: #fff; font-size: 24px; font-weight: 200; letter-spacing: 8px; text-transform: uppercase; opacity: 0.8; margin-top: 10px; background: linear-gradient(to right, #fff, #eca0ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
      ...
      <div class="ag-splash-content">
        ${l}
        <div class="ag-splash-title">Spotify +</div>
      </div>
      <div class="ag-splash-footer">Made by Takayashii</div>
      <div style="position: absolute; bottom: 20px; left: 20px; color: #fff; font-size: 10px; letter-spacing: 1px; font-family: monospace;">Here for you <3</div>
    `;
...

    {
      ...
      style.textContent = `
        ...
        .ag-fixed-hitbox:hover {
          background: rgba(255, 255, 255, 0.15) !important;
          border-color: rgba(255, 255, 255, 0.2) !important;
          box-shadow: 0 0 20px rgba(219, 95, 219, 0.3);
        }
  ...
  function ensureSettingsDockButton() {
    ...
    if (!button) {
      ...
      button.setAttribute('aria-label', 'Spotify+ Settings');
      button.title = 'Spotify+ Settings';
  ...
  async function updateSearchPageRedesign() {
    ...
    eyebrow.textContent = 'Inteligent Search';
  ...
  function ensureLayoutEditorStyle() {
    ...
    style.textContent = `
      ...
      #ag-layout-save { background: var(--ag-accent-main, #db5fcb) !important; }
      .ag-layout-handle { position: fixed; z-index: 99999; display: flex; align-items: center; justify-content: center; min-width: 96px; height: 28px; padding: 0 10px; border-radius: 999px; border: 1px solid rgba(255,255,255,0.22); color: #fff; background: rgba(215, 95, 219, 0.78); box-shadow: 0 8px 24px rgba(0,0,0,0.35); font-size: 12px; font-weight: 800; cursor: grab; pointer-events: auto; user-select: none; }
  ...
  function startLayoutEditor() {
    ...
    overlay.querySelector('#ag-layout-save').addEventListener('click', () => {
      ...
      stopLayoutEditor();
      Spicetify.showNotification("Spotify+: Layout saved");
    });
  ...
  const DEFAULTS = {
    glow: 'rgb(255, 184, 242)', s1: 'rgb(34, 25, 41)', s2: 'rgb(205, 155, 188)', accent: 'rgb(255, 183, 221)',
    home: 'rgb(255, 182, 220)', liked: 'rgb(255, 182, 220)', albums: 'rgb(255, 182, 220)', playlists: 'rgb(255, 182, 220)', artists: 'rgb(255, 182, 220)',
    btn_bg: 'rgba(44, 30, 50, 0.65)', btn_icon: '#f0f0ff', blobs: '#cd5fdb',
    now_mode: 'colored', now_gradient_enabled: 'false', now_color: 'rgb(59, 38, 64)', now_color_2: '#7a3d77', now_gradient_angle: '135', now_gradient_position: '50', now_gradient_blend: '36', now_brightness: '72', now_frost: '52',
    now_width: '860', now_height: '80', now_radius: '40', cover_spin: 'false',
    sidebar_song_bg: 'false', sidebar_bg_strength: '60',
    page_song_bg: 'true',
    glow_opacity: '0.8', glow_size: '195', blobs_opacity: '0.25',
  ...
  const ONBOARDING_SLIDES = [
    {
      kicker: 'Riddle For You',
      title: 'Hmmmm',
      text: 'There is this girl, she has gorgeous eyes.'
    },
    {
      kicker: 'Riddle For You',
      title: 'Hint 1',
      text: 'She has a really beautiful smile.'
    },
    {
      kicker: 'Riddle For You',
      title: 'Hint 2',
      text: 'Her hair is the prettiest in the whole world.'
    },
    {
      kicker: 'Riddle For You',
      title: 'Hint 3',
      text: 'She thinks that she is weird, but I love her personality.'
    },
    {
      kicker: 'Riddle For You',
      title: 'Final Hint',
      text: 'And she is reading all this right now.'
    }
  ];
  ...
  function showOnboarding(force = false) {
    ...
    overlay.innerHTML = `
      ...
      <div class="ag-onboarding-card">
        ...
        <div class="ag-onboarding-mark" aria-hidden="true">M</div>
  ...
  function openSettingsPanel() {
        ...
        <div class="ag-settings-content">
          ...
          <div class="ag-settings-tabs">
            ...
            <div class="ag-settings-body">
              ...
              <div class="ag-setting-item"><span class="ag-setting-label"><label>Frosted Glass Blur</label>${tip('Controls blur amount for Spotify+ glass panels.')}</span><input type="range" id="ag-range-glass_blur" min="0" max="80" step="2" value="${getSetting('glass_blur')}"/></div>
              ...
              <div class="ag-setting-item"><span class="ag-setting-label"><label>Introduction</label>${tip('Shows the Spotify+ first-run introduction again.')}</span><button id="ag-show-onboarding" type="button">Show</button></div>
            </div>
            ...
            <div class="ag-tab-content" id="tab-ui">
              <div class="ag-gradient-builder" data-gradient-enabled="${isNowGradientEnabled()}" data-now-mode="${getSetting('now_mode')}">
               ...
               <p style="font-size: 11px; opacity: 0.6; margin-top: 10px;">These options use Spotify+ variables only and do not change Spotify spice text colors.</p>
            </div>
          </div>
    ...
    d.querySelector('#ag-reset-layout')?.addEventListener('click', () => {
      ...
      applyLayout(getLayout());
      updateVisibility();
      Spicetify.showNotification("Spotify+: Layout reset");
    });
    ...
    d.querySelector('#ag-save-settings').addEventListener('click', () => {
      ...
      document.body?.classList.remove('ag-settings-open');
      setNowPlayingPreview(false);
      Spicetify.showNotification("Spotify+: Applied");
    });
    d.querySelector('#ag-reset-settings').addEventListener('click', () => {
      ...
      document.body?.classList.remove('ag-settings-open');
      setNowPlayingPreview(false);
      Spicetify.showNotification("Spotify+: Reset to defaults");
    });
  }
  ...
  function applyCustomColors() {
    ...
    const nowGlowGlass = 'linear-gradient(90deg, var(--ag-surface-tint-soft, rgba(194, 95, 219, 0.22)), color-mix(in srgb, var(--ag-s2, #db4fd4) 18%, transparent))';
    ...
    root.style.setProperty('--ag-now-glass-glow', nowMode === 'gradient'
      ...
      : 'var(--ag-surface-tint-soft, rgba(219, 95, 213, 0.18))');
    ...
    s.textContent = `
      ...
      #ag-jam-floating-pill {
        ...
        background: rgba(205, 95, 219, 0.72) !important;
        color: #fff !important;
        font-weight: 800 !important;
        font-size: 12px !important;
        box-shadow: 0 10px 30px rgba(0,0,0,0.35), 0 0 18px rgba(198, 95, 219, 0.28) !important;
      }
      ...
      #ag-settings-panel #ag-reset-layout {
        ...
        background: rgba(219, 95, 209, 0.72) !important;
        color: #fff !important;
        font-weight: 800 !important;
  ...
  function injectSettingsToMenu() {
    ...
    if (menu && !menu.querySelector('#ag-menu-item')) {
      ...
      item.innerHTML = `<button class="main-contextMenu-menuItemButton"><span class="main-contextMenu-listItemText">Spotify+ Settings</span>${svg(ICONS.home, {size:16})}</button>`;
...
// ==================== TOPBAR BUTTON ====================
    function createButton() {
        ...
        if (Spicetify.Topbar?.Button) {
            ...
            window.lyricMiniplayerButtonLoaded = true;
        }
    }
